...
Result<std::filesystem::path> relative_staged_path(const std::filesystem::path &root, const std::filesystem::path &path) {
    for (const auto &component : path) {
        if (!path_component_is_safe(component)) {
            return std::unexpected(make_error(ErrorKind::policy, "image archive path must not contain '.' or '..': " + path.string()));
        }
    }
        ...
        for (; root_it != normalized_root.end(); ++root_it, ++path_it) {
            if (path_it == normalized.end() || *path_it != *root_it) {
                return std::unexpected(
                    make_error(ErrorKind::policy, "image archive path is outside configured staging root: " + normalized.string()));
            }
        }
    ...
    for (const auto &component : normalized) {
        if (!path_component_is_safe(component)) {
            return std::unexpected(
                make_error(ErrorKind::policy, "image archive path must not contain '.' or '..': " + normalized.string()));
        }
    }
    return normalized;
}
...
Result<void> validate_owner_if_required(int fd, const QuadletInstallLayout &layout, std::string_view description) {
    struct stat st{};
    if (fstat(fd, &st) != 0) {
        return std::unexpected(
            make_error(ErrorKind::filesystem, "failed to stat " + std::string{description} + ": " + std::strerror(errno), 0, errno));
    }
    if (layout.required_owner_uid && st.st_uid != *layout.required_owner_uid) {
        return std::unexpected(make_error(ErrorKind::filesystem, std::string{description} + " owner uid " + std::to_string(st.st_uid) +
    ...
    if (layout.required_owner_gid && st.st_gid != *layout.required_owner_gid) {
        return std::unexpected(make_error(ErrorKind::filesystem, std::string{description} + " owner gid " + std::to_string(st.st_gid) +
...
Result<OpenArchive> open_staged_image_archive(const ImageArchive &archive, const DeploymentOptions &options) {
    if (!archive.expected_sha256.empty()) {
        return std::unexpected(make_error(ErrorKind::policy, "ImageArchive::expected_sha256 is not implemented; verify bundle "
                                                             "manifest digests before deployment"));
    }

    if (!options.image_archive_root) {
        return std::unexpected(make_error(ErrorKind::policy, "deployment image loading requires DeploymentOptions::image_archive_root"));
    }
    ...
    FileDescriptor current{open(options.image_archive_root->c_str(), O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW)};
    if (current.get() < 0) {
        return std::unexpected(make_error(
            ErrorKind::filesystem,
            "failed to open image archive staging root '" + options.image_archive_root->string() + "': " + std::strerror(errno), 0, errno));
    }
        ...
        if (final) {
            ...
            if (fstatat(current.get(), name.c_str(), &st, AT_SYMLINK_NOFOLLOW) != 0) {
                return std::unexpected(make_error(ErrorKind::filesystem,
                                                  "failed to stat staged image archive component '" + name + "': " + std::strerror(errno),
                                                  0, errno));
            }
            if (!S_ISREG(st.st_mode)) {
                return std::unexpected(make_error(ErrorKind::policy, "staged image archive must be a regular file"));
            }
            if (st.st_size < 0 || std::cmp_greater(st.st_size, options.max_image_archive_bytes)) {
                return std::unexpected(make_error(ErrorKind::policy, "staged image archive is too large"));
            }
        }

        FileDescriptor next_fd{
            openat(current.get(), name.c_str(),
                   final ? (O_RDONLY | O_CLOEXEC | O_NOFOLLOW | O_NONBLOCK) : (O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW))};
        if (next_fd.get() < 0) {
            return std::unexpected(make_error(
                ErrorKind::filesystem, "failed to open staged image archive component '" + name + "': " + std::strerror(errno), 0, errno));
        }
    ...
    if (fstat(current.get(), &st) != 0) {
        return std::unexpected(
            make_error(ErrorKind::filesystem, "failed to stat staged image archive: " + std::string{std::strerror(errno)}, 0, errno));
    }
    if (!S_ISREG(st.st_mode)) {
        return std::unexpected(make_error(ErrorKind::policy, "staged image archive must be a regular file"));
    }
...
class DeploymentLock {
  public:
    ...
    explicit DeploymentLock(FileDescriptor fd) noexcept : fd_{std::move(fd)} {}
...
Result<DeploymentLock> lock_deployment(const QuadletInstallLayout &layout, uid_t uid, std::string_view unit) {
    std::error_code ec;
    std::filesystem::create_directories(layout.admin_user_root, ec);
    if (ec) {
        return std::unexpected(make_error(ErrorKind::filesystem, "failed to create deployment lock root '" +
                                                                     layout.admin_user_root.string() + "': " + ec.message()));
    }

    FileDescriptor root_fd{open(layout.admin_user_root.c_str(), O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW)};
    if (root_fd.get() < 0) {
        return std::unexpected(
            make_error(ErrorKind::filesystem,
                       "failed to open deployment lock root '" + layout.admin_user_root.string() + "': " + std::strerror(errno), 0, errno));
    }
    ...
    if (mkdirat(root_fd.get(), ".locks", 0755) != 0 && errno != EEXIST) {
        return std::unexpected(make_error(ErrorKind::filesystem,
                                          "failed to create deployment lock directory: " + std::string{std::strerror(errno)}, 0, errno));
    }

    FileDescriptor dir_fd{openat(root_fd.get(), ".locks", O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW)};
    if (dir_fd.get() < 0) {
        return std::unexpected(
            make_error(ErrorKind::filesystem, "failed to open deployment lock directory: " + std::string{std::strerror(errno)}, 0, errno));
    }
    ...
    if (fchmod(dir_fd.get(), 0755) != 0) {
        return std::unexpected(
            make_error(ErrorKind::filesystem, "failed to chmod deployment lock directory: " + std::string{std::strerror(errno)}, 0, errno));
    }

    const auto     lock_name = std::to_string(uid) + "-" + std::string{unit} + ".lock";
    FileDescriptor lock_fd{openat(dir_fd.get(), lock_name.c_str(), O_RDWR | O_CREAT | O_CLOEXEC | O_NOFOLLOW, 0644)};
    if (lock_fd.get() < 0) {
        return std::unexpected(
            make_error(ErrorKind::filesystem, "failed to open deployment lock '" + lock_name + "': " + std::strerror(errno), 0, errno));
    }
    for (;;) {
        ...
        return std::unexpected(
            make_error(ErrorKind::filesystem, "failed to lock deployment '" + lock_name + "': " + std::strerror(errno), 0, errno));
    }
}

bool deployed_status_is_healthy(const UnitStatus &status) { return status.load_state == "loaded" && status.active_state == "active"; }

std::string describe_status(const UnitStatus &status) {
    return "LoadState=" + status.load_state + " ActiveState=" + status.active_state + " SubState=" + status.sub_state;
}

template <typename T>
Result<T> rollback_error(const QuadletInstaller &installer, const UserSystemdController &systemd, const PodmanTarget &target, uid_t uid,
                         const QuadletSnapshot &snapshot, std::string_view unit, bool restart_restored_unit, Error original) {
    ...
    if (!rollback) {
        ...
        if (auto reload = systemd.daemon_reload(target); !reload) {
            ...
            if (!restart) {
                ...
                original.message += "; rollback restart left unit unhealthy: " + describe_status(restart->final_status);
            }
        }
    }
...
Result<void> validate_deployment_bundle(const DeploymentBundle &bundle) {
    if (bundle.target_uid == 0) {
        return std::unexpected(make_error(ErrorKind::policy, "deployment target must be a non-root user uid"));
    }
    if (bundle.service_name.empty()) {
        return std::unexpected(make_error(ErrorKind::invalid_argument, "deployment service_name is empty"));
    }
    if (auto unit = service_unit_name_from_quadlet(bundle.quadlet.file_name); !unit) {
        return std::unexpected(unit.error());
    } else if (*unit != bundle.service_name + ".service") {
        return std::unexpected(make_error(ErrorKind::invalid_argument, "deployment service_name '" + bundle.service_name +
                                                                           "' does not match Quadlet file '" + bundle.quadlet.file_name +
                                                                           "'"));
    }
    if (bundle.image_archive && bundle.image_archive->path.empty()) {
        return std::unexpected(make_error(ErrorKind::invalid_argument, "image archive path is empty"));
    }
    return {};
}
...
DeploymentOrchestrator::DeploymentOrchestrator(QuadletInstaller installer, std::shared_ptr<UserSystemdController> systemd,
                                               DeploymentOptions options)
    : installer_{std::move(installer)}, systemd_{std::move(systemd)}, options_{std::move(options)} {}

Result<DeploymentResult> DeploymentOrchestrator::deploy(const DeploymentBundle &bundle) const {
    if (!systemd_) {
        return std::unexpected(make_error(ErrorKind::invalid_argument, "deployment orchestrator requires a systemd controller"));
    }
    ...
    if (auto reload = systemd_->daemon_reload(*target); !reload) {
        return rollback_error<DeploymentResult>(installer_, *systemd_, *target, bundle.target_uid, *snapshot, result.systemd_unit, false,
                                                reload.error());
    }
    ...
    if (options_.restart_unit) {
        ...
        if (!operation) {
            return rollback_error<DeploymentResult>(installer_, *systemd_, *target, bundle.target_uid, *snapshot, result.systemd_unit, true,
                                                    operation.error());
        }
        ...
        if (!deployed_status_is_healthy(operation->final_status)) {
            return rollback_error<DeploymentResult>(
                installer_, *systemd_, *target, bundle.target_uid, *snapshot, result.systemd_unit, true,
                make_error(ErrorKind::systemd, "deployment unit is unhealthy after restart: " + describe_status(operation->final_status)));
        }
        return result;
    }
