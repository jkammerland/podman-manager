...
struct QuadletInstallLayout {
    ...
    size_t                max_quadlet_bytes{static_cast<size_t>(1024) * 1024};
...
class ParsedQuadlet {
  public:
    ...
    [[nodiscard]] std::vector<std::string>              values(std::string_view section, std::string_view key) const;
...
class QuadletInstaller {
  public:
    explicit QuadletInstaller(QuadletInstallLayout layout = {}, QuadletPolicy policy = {});
